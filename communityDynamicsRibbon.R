...
# calculates the average abundance of the pupolations in time over different runs of the same batch of 
...
for (i in c(1:lengthList)) { # should become a plyr function
        matcol[[i]]<-dataList[[i]][,c(1,4,6,8,10,12,14,20)] # list of matrix containing data of interest: time and classes for each .csv
}
...
ribbonAbundance <- ggplot(complete, aes(x=time, y=mean, group=class))+
  ...
  scale_y_continuous(name="Abundance [individuals]", 
                     limits=c(1,1000000),
...
ggsave("/home/somros/Documents/ITNFollowUp/picsWP/communityAbundance.pdf", ribbonAbundance, useDingbats=FALSE ) # set better res pls
