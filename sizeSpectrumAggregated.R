...
colnames(meltComplete) <- c("logBins", "Regime", "Frequency")
...
spectrumPlot <- ggplot(data=meltComplete, #subset(meltComplete, Regime=="base" | Regime=="U_I3" |
...
spectrumPlot <- ggplot(data=subset(meltComplete, Regime=="base" | Regime=="U_I3" |
                       Regime=="S500_I3" | Regime=="S250_I3"),
                       aes(x=logBins, y=Frequency, group=Regime))+
  geom_smooth(span=.1, colour="black", size=.4)+
  geom_point(size=.4)+
  scale_y_continuous(name="log(number of individuals)", 
                     limits=c(0,12),
                     breaks=seq(0,12,2))+
...
spectrumPlot <- ggplot(data=subset(meltComplete, Regime=="base" | Regime=="U_I3" |
                       Regime=="S500_I3" | Regime=="S250_I3"),
                       aes(x=logBins, y=Frequency, group=Regime))+
  geom_smooth(aes(linetype=Regime), method="lm")+
  scale_y_continuous(name="log(number of individuals)", 
                     limits=c(0,10),
                     breaks=seq(0,10,.5))+
  # scale_shape_manual(name="Class",
...
fitter <- function(data) {
  fitExperiment <- lm(data$ln_freq ~ data$ln_length)#, 
  #weights=ensemble$ln_freq) # fitting the quadratic model to the AVERAGE bins
  #plot(ensemble$ln_length, ensemble$ln_freq)
  # newx <- data.frame(bin = data$ln_length)
  # pred <- predict(fitExperiment, newdata=newx) # fit regression model to whichever x and get the y
  # pdat <- data.frame(newx, pred, ymax=pred+data$sd, ymin=pred-data$sd) # create array of coordinates, y of the model is y, the bins are x (x could be whatever, just the length must be the same)
  # pdat <- with(data.frame(pred),
  #              data.frame(x = newx, y = fitExperiment))
  coefficients <- coef(summary(fitExperiment))[,1]
  error <- coef(summary(fitExperiment))[,2]
  rsquared <- summary(fitExperiment)$r.squared
  dof <- summary(fitExperiment)$df
  return(list(coefficients, error, rsquared, dof))
}

regParameters <- lapply(dataList, fitter)
unlist(regParameters)
