...
relativeBiomassMean <- as.data.frame(matrix(nrow=nrow(meanValues), ncol=ncol(meanValues)))
for (i in 1:nrow(meanValues)) {
  relativeBiomassMean[i,] <- ((meanValues[i,]/meanValues[1,])-1)*100
}

colnames(relativeBiomassMean) <- headers
relativeBiomassMean$Regime <- meanValues$Regime

# sd according to propagation of error

relativeBiomassSd <- as.data.frame(matrix(nrow=nrow(sdValues), ncol=ncol(sdValues)))
for (i in 1:nrow(sdValues)) {
  relativeBiomassSd[i,] <- abs(((sqrt((sdValues[i,]/meanValues[i,])^2+(sdValues[1,]/meanValues[1,])^2))-1)*100)
}

colnames(relativeBiomassSd) <- headers
relativeBiomassSd$Regime <- meanValues$Regime

meltmeanValues <- melt(relativeBiomassMean, id.vars="Regime")
meltsdValues <- melt(relativeBiomassSd, id.vars="Regime")
...
newPlot <- ggplot(data=subset(cleanData, Regime=="U_I2" | Regime=="U_I3" | Regime=="S500_I2" |
  ...
  scale_y_continuous(limits=c(-300,300),
                     ...
                     expand=c(0,0), labels=seq(-300,300,50), "Biomass variation from baseline [%]")+
...
ggsave("/home/somros/Documents/ITNFollowUp/picsWP/relativeBiomass.pdf", newPlot, 
